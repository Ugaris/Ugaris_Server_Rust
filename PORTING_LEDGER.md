...
| `src/system/arena.c` | `crates/ugaris-core/src/player.rs`, `crates/ugaris-core/src/item_driver/arena.rs`, `crates/ugaris-core/src/world/arena.rs`, `crates/ugaris-core/src/character_driver.rs`, `crates/ugaris-core/src/zone.rs`, `crates/ugaris-server/src/world_events.rs`, `crates/ugaris-server/src/main.rs` | The per-character `arena_ppd` data model, the pure `score_fight` win/loss/score math (`PlayerRuntime::arena_fight_worth`/`record_arena_fight_result`, now split into `apply_arena_win`/`apply_arena_loss` halves - see the "Continuation Handoff" bullet below), `master_driver` (`CDR_ARENAMASTER = 48`), and `fighter_driver` (`CDR_ARENAFIGHTER = 49`, the autonomous practice-bot) are all ported and wired into the live tick loop: contender registration/pairing (`add_contender`/`find_contender`), the box-entry gate and fight-timeout/win/loss/draw detection (`check_inside`/`check_fight`), the post-fight kick-out (`empty_arena`), `enter`/`leave` text commands, `NT_GIVE` handling, the server-wide `update_toplist` ranking table (`World::arena_update_toplist`/`arena_toplist_entries`, in-memory only - no DB/file persistence yet, same gap as `MilitaryMasterStorageRegistry`), and the fighter bot's own `FS_LEISURE`/`FS_START`/`FS_REGISTER`/`FS_WAIT`/`FS_ENTER`/`FS_WAIT2`/`FS_FIGHT` state machine (narrowed to a single tracked enemy instead of the generic `fight_driver_data`, same precedent as `world/gate_fight.rs`; its own win/loss ledger lives directly on `ArenaFighterDriverData` since it has no `PlayerRuntime`, resetting on respawn/restart - the same class of gap as the ranking table's own missing persistence). `apply_arena_master_events` now falls back to that local ledger whenever a `FightScored` participant isn't a real player, instead of silently dropping the event. `main.rs`'s `ArenaToplist` item-driver outcome now renders real `arena_toplist_lines` output instead of emitting nothing. REMAINING: `manager_driver` (`CDR_ARENAMANAGER`, the paid arena-rental system, still an unported NPC state machine) and DB/file persistence for the ranking table and the fighter bot's own local ledger. See `PORTING_TODO.md`'s "Arena rankings" P3 entry for the itemized remaining list. |
