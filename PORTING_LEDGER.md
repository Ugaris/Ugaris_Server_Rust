...
| `src/system/arena.c` | `crates/ugaris-core/src/player.rs`, `crates/ugaris-core/src/item_driver/arena.rs` | First self-contained slice on top of the already-ported `IDR_TOPLIST` formatter (see the "Continuation Handoff" bullet below): the per-character `arena_ppd` data model and the pure `score_fight` win/loss/score math, with zero NPC/networking surface. `PlayerRuntime::arena_ppd: Vec<u8>` (`LEGACY_ARENA_PPD_SIZE` = 20 bytes, `arena.c:204-211`'s `score`/`fights`/`wins`/`losses`/`lastfight`) uses the same raw-block-with-offset-accessor pattern as `area3_ppd`, wired into `decode_legacy_ppd_blob`/`encode_legacy_ppd_blob`; `arena_score()` reproduces C's `!ppd->fights` read-time re-seed-to--2000 quirk. `PlayerRuntime::arena_fight_worth` ports the 30-branch `diff`->`worth` ELO-like ladder (`arena.c:451-524`, unit tested at every boundary) and `PlayerRuntime::record_arena_fight_result` ports `score_fight`'s per-character mutation (`arena.c:432-534`, minus the `update_toplist` ranking-table call). `DRD_ARENA_PPD` graduated out of `clear_turn_seyan_ppd`'s raw-block `strip_ppd_blocks` list to a real `.clear()`. 9 new unit tests. REMAINING: the entire tournament NPC state machine that triggers an arena kill (`master_driver`/`fighter_driver`/contender pairing/fight-timeout detection, `arena.c:222-1039` - no Rust equivalent exists at all), the server-wide `struct toplist`/`update_toplist` 100-entry ranking table and its file/blob persistence (needs an architectural decision - `ugaris-db` has no generic "storage blob" concept), and wiring the already-ported `arena_toplist_lines`/`toplist_driver` to real data (`main.rs`'s `ArenaToplist` handler still emits nothing). See `PORTING_TODO.md`'s "Arena rankings" P3 entry for the itemized remaining list. |
