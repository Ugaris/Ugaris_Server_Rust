...
- `IDR_NIGHTLIGHT = 11`, `IDR_TORCH = 12`, and `IDR_TOYLIGHT = 117` now dispatch from the base item-driver path for core state transitions: toy lights toggle `drdata[0]`/sprite/light modifier, nightlights respond to timer/daylight threshold context and request 30-second rescheduling, torches light/extinguish carried items, set `IF_NODECAY`, use the legacy light fade formula, mark modified torches with `min_level = 200`, handle underwater block/extinguish context, request 30-second timer rescheduling, and destroy/remove expired torches from carried inventory or map storage through world timer callbacks with focused core tests. Startup scheduling for existing nightlights/burning torches, item-light map invalidation, character stat recomputation, orb extraction from modified torches, and client feedback text remain.
...
- Finish runtime integration for `IDR_NIGHTLIGHT`, `IDR_TORCH`, and `IDR_TOYLIGHT`: startup scheduling for existing timer-driven lights, map light invalidation, update-char/stat recomputation, torch modifier orb extraction, and exact feedback text.
- Finish enchant/orb family details: extracting anti-orb item creation, exact feedback/look output, requirement recomputation, `IDR_ORBSPAWN`, `IDR_ANTIORBSPAWN`, and legacy `DRD_ORBSPAWN_PPD` cooldown persistence.
