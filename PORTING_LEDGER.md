...
| `src/system/arena.c` | `crates/ugaris-core/src/player.rs`, `crates/ugaris-core/src/item_driver/arena.rs`, `crates/ugaris-core/src/world/arena.rs`, `crates/ugaris-core/src/character_driver.rs`, `crates/ugaris-server/src/world_events.rs`, `crates/ugaris-server/src/main.rs` | The per-character `arena_ppd` data model, the pure `score_fight` win/loss/score math (`PlayerRuntime::arena_fight_worth`/`record_arena_fight_result`, now split into `apply_arena_win`/`apply_arena_loss` halves - see the "Continuation Handoff" bullet below), and `master_driver` (`CDR_ARENAMASTER = 48`) are all ported and wired into the live tick loop: contender registration/pairing (`add_contender`/`find_contender`), the box-entry gate and fight-timeout/win/loss/draw detection (`check_inside`/`check_fight`), the post-fight kick-out (`empty_arena`), `enter`/`leave` text commands, `NT_GIVE` handling, and the server-wide `update_toplist` ranking table (`World::arena_update_toplist`/`arena_toplist_entries`, in-memory only - no DB/file persistence yet, same gap as `MilitaryMasterStorageRegistry`). `main.rs`'s `ArenaToplist` item-driver outcome now renders real `arena_toplist_lines` output instead of emitting nothing. REMAINING: `fighter_driver`/`manager_driver` (`CDR_ARENAFIGHTER`/`CDR_ARENAMANAGER`, the autonomous practice-bot and the paid arena-rental system, `arena.c:790-1039` - two separate, still-unported NPC state machines) and DB/file persistence for the ranking table. See `PORTING_TODO.md`'s "Arena rankings" P3 entry for the itemized remaining list. |
