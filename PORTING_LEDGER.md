...
| `src/system/do.c` / `src/system/act.c` pure spell formulas and primitive queued spell bridge | `crates/ugaris-core/src/spell.rs`, `crates/ugaris-core/src/do_action.rs`, `crates/ugaris-core/src/world.rs` | Spell constants, effect IDs, spell item driver IDs, spellpower/duration math, variable mana spend formulas, speed modifier formulas, combat spell helper formulas, player queued spell setup for heal/magic shield/pulse, timed magic-shield lifeshield completion, timed heal target HP completion, and no-op pulse completion bridge ported with tests. Full projectile spells, bless/freeze/flash/warcry effects, spell item/effect lifecycle, notifications/sounds, `may_add_spell` recast blocking, and exact visibility/help/attack policy remain. |
...
- Primitive queued spell execution now routes `PAC_HEAL`, `PAC_MAGICSHIELD`, and `PAC_PULSE` through the world action bridge. The core ports C `do_heal`/`act_heal` HP restoration, `do_magicshield`/`act_magicshield` lifeshield restoration, pulse mana/action setup, legacy action IDs/durations, mana spending, fast endurance cost, and target-facing behavior with focused core tests. Projectile/effect-backed spells, bless/freeze/flash/warcry, spell item timers, notifications/sounds, and exact policy checks remain.
...
- Continue spell queue execution beyond the primitive heal/magic-shield/pulse bridge: projectile/effect-backed fireball/ball/flash, bless/freeze/warcry, spell item timers/recast blocking, notifications/sounds, and exact policy checks.
