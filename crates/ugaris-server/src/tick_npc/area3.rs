...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn supermax_driver_85(
    world: &mut World,
    runtime: &mut ServerRuntime,
    _zone_loader: &mut ZoneLoader,
    config: &ServerConfig,
    ...
    // C `supermax_driver`: Aston's past-maxes raiser NPC
    // (`src/area/3/area3.c`).
    let supermax_facts = supermax_player_facts(runtime);
    let supermax_events = world.process_supermax_actions(&supermax_facts, config.area_id);
    let supermax_events_applied = apply_supermax_events(runtime, supermax_events);
    if supermax_events_applied != 0 {
        info!(
            supermax_events_applied,
            tick = world.tick.0,
            "applied supermax dialogue events"
        );
    }
}

#[allow(clippy::too_many_arguments)]
pub(crate) async fn lampghost_driver_86(
    world: &mut World,
    _runtime: &mut ServerRuntime,
    _zone_loader: &mut ZoneLoader,
    config: &ServerConfig,
    ...
    // C `lampghost_driver`: area 3's palace-light puzzle janitor NPC
    // (`src/area/3/area3.c`).
