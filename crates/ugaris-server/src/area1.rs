//! Server-side wiring for area 1's forest hermit and hunter NPCs
...
//! [`yoakin_player_facts`] snapshot the per-player `area1_ppd`/`quest_log`
//! facts each NPC's dialogue needs before the tick, and
//! [`apply_camhermit_events`]/[`apply_yoakin_events`] apply the returned
...
use ugaris_core::world::{
    CamhermitOutcomeEvent, CamhermitPlayerFacts, YoakinOutcomeEvent, YoakinPlayerFacts,
};
