...
use crate::character_driver::{mem_add_driver, mem_check_driver, mem_erase_driver, ClanFoundData};
...
/// itself).
pub(crate) fn take_name_token(text: &str) -> (String, &str) {
...
impl World {
    ...
    pub(crate) fn char_is_clan_or_club_member(&mut self, character_id: CharacterId) -> bool {
        let Some(character) = self.characters.get_mut(&character_id) else {
            return false;
        };
    ...
    pub(crate) fn find_online_player_by_name(&self, name: &str) -> Option<CharacterId> {
