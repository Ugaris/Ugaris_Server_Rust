...
#[test]
fn staffer_ppd_tracks_carlos2_state_independently_of_carlos_state() {
    let mut player = PlayerRuntime::connected(1, 0);
    assert_eq!(player.staffer_carlos_state(), 0);
    assert_eq!(player.staffer_carlos2_state(), 0);

    player.set_staffer_carlos_state(6);
    player.set_staffer_carlos2_state(5);
    assert_eq!(player.staffer_carlos_state(), 6);
    ...
    let mut decoded = PlayerRuntime::connected(2, 0);
    assert!(decoded.decode_legacy_staffer_ppd(&encoded));
    assert_eq!(decoded.staffer_carlos_state(), 6);
